...
const WELCOME = "Hi! I'm the Huit Data Ventures AI Revenue Agent. I can help you find the right plan, answer questions, or walk you through what we do. What brings you here today?";

function CTAButton({ text, href }: { text: string; href: string }) {
  return (
    <a href={href} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#C9A84C",color:"#050D1A",padding:"8px 16px",borderRadius:6,fontSize:13,fontWeight:700,textDecoration:"none",marginTop:8}} onClick={()=>{const btn=document.querySelector("[data-agent-toggle]") as HTMLButtonElement;if(btn)btn.click();}}>
      {text} →
    </a>
  );
}
...
function MessageBubble({ msg }: { msg: Message }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:isAgent?"flex-start":"flex-end",marginBottom:12}}>
      <div style={{
        maxWidth:"85%",
        background:isAgent?"#050D1A22":"#FFFFFF18",
        border:isAgent?`1px solid ${"#C9A84C"}25`:"1px solid #FFFFFF20",
    ...
    <>
      ...
      {open && (
        <div style={{
          ...
          background:"#050D1A", border:`1px solid ${"#C9A84C"}30`,
          ...
          <div style={{
            ...
            borderBottom:`1px solid ${"#C9A84C"}20`,
            background:`${"#C9A84C"}12`, borderRadius:"16px 16px 0 0",
          }}>
            <div style={{width:8,height:8,borderRadius:"50%",background:"#C9A84C",boxShadow:`0 0 8px ${"#C9A84C"}`}} />
            <div>
              <div style={{fontSize:14,fontWeight:700,color:"#F0F4FF"}}>Huit Data Ventures AI Agent</div>
            ...
            {loading && (
              <div style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",fontSize:13,color:"#8FA3C0"}}>
                <div style={{display:"flex",gap:4}}>
                  {[0,1,2].map(i => (
                    <div key={i} style={{width:6,height:6,borderRadius:"50%",background:"#C9A84C",opacity:0.6,animation:`pulse 1.2s ease-in-out ${i*0.2}s infinite`}} />
                  ))}
                </div>
                Thinking...
              </div>
          ...
          {/* Input */}
          <div style={{padding:"12px 16px 16px",borderTop:`1px solid ${"#C9A84C"}20`}}>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <input
                ...
                style={{
                  flex:1,background:"#FFFFFF10",border:`1px solid ${"#C9A84C"}25`,
              ...
              <button
                ...
                style={{
                  background:"#C9A84C",color:"#050D1A",border:"none",borderRadius:8,
            ...
            <div style={{fontSize:11,color:"#8FA3C0",textAlign:"center",marginTop:8}}>Powered by Huit.AI · <a href="/pricing" style={{color:"#C9A84C",textDecoration:"none"}}>View Plans</a></div>
          </div>
        </div>
      )}
      ...
      <button
        ...
        style={{
          ...
          boxShadow:`0 8px 32px ${"#C9A84C"}60`,
          cursor:"pointer", zIndex:9999,
          display:"flex", alignItems:"center", justifyContent:"center",
          fontSize:24, transition:"transform 0.2s",
        }}
        title="Chat with Huit Data Ventures AI Agent"
      >
        {open ? "×" : "💬"}
        {!open && unread > 0 && (
          <div style={{
