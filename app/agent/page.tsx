...
export default function AgentPage() {
  ...
  return (
    <div style={{background:"#050D1A",color:"#F0F4FF",fontFamily:"system-ui,sans-serif",minHeight:"100vh"}}>

      {/* Hero */}
      <section style={{maxWidth:900,margin:"0 auto",padding:"100px 24px 72px",textAlign:"center"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"#C9A84C18",border:"1px solid #C9A84C35",borderRadius:40,padding:"6px 20px",fontSize:12,fontWeight:700,color:"#C9A84C",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:28}}>
          <span style={{width:6,height:6,borderRadius:"50%",background:"#C9A84C",animation:"pulse 2s infinite"}} />
          AI Revenue Workforce — Huit Data Ventures
        </div>
        ...
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href="/pricing" style={{background:"#C9A84C",color:"#050D1A",border:"none",borderRadius:8,padding:"16px 36px",fontSize:16,fontWeight:800,textDecoration:"none",display:"inline-block"}}>Contact Sales →</a>
      ...
      <section style={{borderTop:"1px solid rgba(255,255,255,0.08)",borderBottom:"1px solid rgba(255,255,255,0.08)",padding:"40px 24px",background:"#0A162820"}}>
        <div style={{maxWidth:900,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:40,textAlign:"center"}}>
          {[["3x","More qualified meetings"],["21 hrs","Saved per week on manual work"],["72%","Decrease in response times"],["24/7","Always-on coverage"]].map(([val,label]) => (
            <div key={val}>
              <div style={{fontSize:44,fontWeight:900,color:"#C9A84C",lineHeight:1}}> {val}</div>
      ...
      <section style={{maxWidth:1100,margin:"0 auto",padding:"88px 24px"}}>
        ...
        <div style={{display:"flex",gap:12,justifyContent:"center",marginBottom:40,flexWrap:"wrap"}}>
          {AGENTS.map(a => (
            <button key={a.id} onClick={()=>setActive(a.id)} style={{
              background:active===a.id?"#C9A84C":"transparent",
              color:active===a.id?"#050D1A":"#8FA3C0",
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:40,alignItems:"start"}}>
          <div>
            ...
            <div style={{fontSize:16,color:"#C9A84C",fontWeight:700,marginBottom:20}}> {agent.role}</div>
            <p style={{fontSize:16,color:"#8FA3C0",lineHeight:1.8,marginBottom:32}}> {agent.desc}</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:20,marginBottom:32}}>
              {agent.stats.map(([val, label]) => (
                <div key={val} style={{textAlign:"center",background:"#C9A84C10",border:"1px solid #C9A84C20",borderRadius:12,padding:20}}>
                  <div style={{fontSize:28,fontWeight:900,color:"#C9A84C",lineHeight:1}}> {val}</div>
                  <div style={{fontSize:12,color:"#8FA3C0",marginTop:6,lineHeight:1.4}}> {label}</div>
                </div>
              ))}
            </div>
            <a href="/pricing" style={{background:"#C9A84C",color:"#050D1A",padding:"14px 32px",borderRadius:8,fontWeight:700,textDecoration:"none",display:"inline-block"}}>Get Started →</a>
          </div>
          <div style={{background:"#0A1628",border:"1px solid rgba(255,255,255,0.08)",borderRadius:16,padding:32}}>
            <div style={{fontSize:14,fontWeight:700,color:"#C9A84C",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:24}}>Capabilities</div>
            {agent.capabilities.map(cap => (
              <div key={cap} style={{display:"flex",gap:12,alignItems:"center",marginBottom:16,fontSize:15,color:"#F0F4FF"}}>
                <span style={{color:"#C9A84C",fontSize:18}}>✓</span> {cap}
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{background:"#0A162820",borderTop:"1px solid rgba(255,255,255,0.08)",borderBottom:"1px solid rgba(255,255,255,0.08)",padding:"80px 24px"}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:24}}>
            {USE_CASES.map(uc => (
              <div key={uc.title} style={{background:"#050D1A",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:28}}>
      ...
      <section style={{maxWidth:700,margin:"0 auto",padding:"88px 24px",textAlign:"center"}}>
        ...
        <p style={{fontSize:17,color:"#8FA3C0",marginBottom:40,lineHeight:1.6}}>Three AI agents working around the clock to build pipeline, qualify leads, and keep you ahead of every shift. Pricing starts at Custom.</p>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href="/pricing" style={{background:"#C9A84C",color:"#050D1A",padding:"18px 48px",borderRadius:8,fontWeight:800,textDecoration:"none",display:"inline-block",fontSize:17}}>Contact Sales →</a>
